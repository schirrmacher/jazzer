...
public class Replayer {
  ...
  static {
    try {
      NativeUtils.loadLibraryFromJar("/com/code_intelligence/jazzer/replay/libnative.so");
    } catch (Exception e) {
      e.printStackTrace();
  ...
  public static void main(String[] args) {
    if (args.length < 2) {
      System.err.println(
          "Usage: <fuzz target class> <input file path> <fuzzerInitializer args>...");
      System.exit(STATUS_OTHER_ERROR);
    }
  ...
  public static void executeFuzzTarget(Class<?> fuzzTarget, byte[] input) {
    ...
    System.err.println("Failed to find fuzzerTestOneInput");
    System.exit(STATUS_OTHER_ERROR);
  }
