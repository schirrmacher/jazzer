...
The build may fail with the clang shipped with Xcode. If you encounter issues during the build, add `--config=ci` right
after `run` or `build` in the `bazelisk` commands above to use a checked-in toolchain that is known to work.
